...
export class AppointmentsService {
  ...
  async list(
    ...
    if (query.providerId) {
      qb.andWhere('appointment.provider_id = :providerId', { providerId: query.providerId });
    }
    if (query.patientId) {
      qb.andWhere('appointment.patient_id = :patientId', { patientId: query.patientId });
    }

    const sortBy = SORT_ALLOW_LIST.includes(query.sortBy) ? query.sortBy : 'date';
  ...
  async create(dto: CreateAppointmentDto, tenant: TenantContext, user: RequestUser) {
  ...
  async checkIn(id: string, dto: CheckInAppointmentDto, tenant: TenantContext, user: RequestUser) {
    const appointment = await this.findOneScoped(id, tenant);
  ...
  async transition(
    ...
    if (status === 'CANCELLED' && ['COMPLETED', 'CANCELLED'].includes(appointment.status)) {
      throw new BadRequestException(`Appointment in status ${appointment.status} cannot be cancelled`);
    }
